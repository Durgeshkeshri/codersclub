import event from '../../ImagesCC/img2.jpg';
...
const Event4 = () => {
  ...
  { name: 'Manisha Devadiga', role: 'Core Member', image: manisha },
  { name: 'Yash Khatra', role: 'Core Member', image: yash },
  { name: 'Kabirji Vishwakarma', role: 'Core Member', image: kabir },
  { name: 'Vasanta Amogth', role: 'Core Member', image: vasanta },
  ...
  return (
    <div className="text-white sm:px-[3%]">
      <div className='flex flex-col tab:flex-row justify-center xl:px-[8%] tab:px-[6%] tab:gap-8 lap:gap-0 max-tab:px-[6%] max-tab:gap-4 mt-6'>
        ...
        <div className='xl:pr-[0%] lap:px-[3%] flex flex-col gap-3 tab:ml-0 max-tab:text-left'>
          <h5 className='font-bold lap:text-[51px] tab:text-[31px] text-[26px] max-smvideo:text-[22px] bigpc:text-[70px]'>
            Alkemy <span className='text-[#0080FF]'>DMS</span>: A Celebration of Coding Excellence
          </h5>
          <p className='lap:text-[21px] tab:text-[17px] text-[13px] bigpc:text-[35px]'>
            Date Held: <span className='text-[#0080FF]'>March-5-2k24</span>
          </p>
          <p className='lap:text-[21px] tab:text-[17px] text-[13px] bigpc:text-[35px]'>
          Join us in celebrating the incredible talent and innovation, From intense coding challenges to groundbreaking projects, relive the excitement and highlights of this unforgettable event,This event was the part of Alkemy Annual Fest.
          </p>
        </div>
      </div>
      ...
      <div className="lap:text-[22px] tab:text-[19px] text-[13px] px-[6%] xl:px-[8%] flex flex-col gap-3 mt-6 ">
        ...
        <p className='mt-1'>
        Alkemy-DMS is a coding competition organized by the Coder's Club, where individual participants tackle four progressively difficult coding challenges. The top performers will advance to a rapid-fire round. Participants code directly on the platform, which supports over 30 programming languages, with browsing prohibited. Built-in library functions are not allowed for the first two questions. Scoring is based on the number of test cases passed, and ties are broken by the time of the last correct submission. Winners will receive electronic gifts and certificates, while all participants earn e-certificates. Alkemy-DMS offers a chance to challenge yourself, compete for prizes, and grow your coding skills in a collaborative environment.</p>
      ...
      <div className="xl:mx-[8%] mx-[6%] p-4 mt-6 bg-slate-900">
        ...
        <div>
          ...
          <table className='w-full mt-4 border-collapse border border-[#0080FF]'>
            ...
            <tbody>
              {[
                { rank: '1st', name: 'Danish Shaikh'},
                { rank: 'First Runner-Up', name: 'STU Of TBV'},
                { rank: 'Second Runner-Up', name: 'Priyanshu Jash'},
                { rank: 'Third Runner-up', name: 'Jyotiaditya Mourya'},
              ].map((participant, index) => (
                <tr key={index}>
                  <td className='tab:text-[16px] text-[13px] max-sm:p-1 p-2 border-r border-[#0080FF]'>{participant.rank}</td>
                  <td className='tab:text-[16px] text-[13px] w-auto max-sm:p-1 p-2 border-r border-[#0080FF]'>{participant.name}</td>
                </tr>
              ))}
            </tbody>
          </table>
